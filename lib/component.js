...
import { REGEX, capitalizeFirstLetter } from './utils';
...
const replaceComponentDeclaration = (html = ""/* , children = "" */) => {
  ...
  componentsInHTML.forEach(componentMatched => {
    ...
    const cc = ComponentClasses.find(cc => {
      const componentClassName = String(cc).match(REGEX.CLASS_NAME)[4].toLowerCase();
      return componentClassName === componentName;
    });
